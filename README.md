...
5. **Compliance Module** — Maps each AI system to **41 testable AI-governance controls** taken verbatim from ISO 42001:2023 (11), EU AI Act (7), NIST AI RMF 1.0 (6), DPDPA 2023 (3), RBI IT Governance 2023 (4), IRDAI ICS Guidelines 2023 (5), and SEBI CSCRF 2024 (5). Every control declares a non-empty `auto_check` predicate that AEGIS can evaluate against Registry, AISIA, integration metadata, and usage telemetry — paper-only artefacts (board minutes, training records, etc.) are deliberately out of scope. Auto-assessment never produces a `not_assessed` verdict: every control receives `partial` / `not_implemented` (or human-attested `implemented` / `not_applicable`), and every mapping carries a pipe-separated `PREDICATE: VERDICT — observation` explanation plus machine-readable `evidence_refs[]` so auditors see exactly why a control passed or failed. Requirement text + citation are deterministic — the importer prints a SHA-256 over the inventory and a CI test asserts it matches the pinned `catalogue/compliance-frameworks/.inventory_digest`. No LLM anywhere in the compliance path.
